...
typedef struct _AHCI_INTERFACE {
  ...
  PAHCI_PORT_REGISTERS      AhciPortControl;            // ”казатель на регистры управлени€ текущим каналом (Port control registers [x])
  //PAHCI_COMMAND_TABLE     AhciCommandTable[32];       // ”казатели на командные таблицы (&AhciCommandTable[0] == AhciBuffer)
  PAHCI_COMMAND_LIST        CmdListBaseAddress;         // ”казатель на список командных слотов (AHCI Command Slots)
